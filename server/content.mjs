...
export const cleanArticleHtml = html => sanitizeHtml(html, {
  allowedTags: ['p', 'br', 'h2', 'h3', 'strong', 'em', 'u', 's', 'blockquote', 'ol', 'ul', 'li', 'a'],
  allowedAttributes: { a: ['href', 'title', 'rel'] },
  allowedSchemes: ['https', 'http', 'mailto'], allowProtocolRelative: false,
  transformTags: { h1: 'h2', b: 'strong', i: 'em', a: sanitizeHtml.simpleTransform('a', { rel: 'noopener noreferrer' }) },
});
export const articleHtml = post => post.contentFormat === 'html' ? cleanArticleHtml(post.content) : markdown(post.content);
export const articleText = post => sanitizeHtml(articleHtml(post).replace(/<\/(?:p|h2|h3|li|blockquote)>/g, ' '), { allowedTags: [], allowedAttributes: {} }).replace(/&nbsp;/g, ' ');
export function validatePost(input, existing) {
  ...
  if (input.contentFormat !== undefined && !['markdown', 'html'].includes(input.contentFormat)) throw new Error('Invalid content format.');
  post.contentFormat = input.contentFormat || 'markdown';
  if (post.contentFormat === 'html') {
    post.content = cleanArticleHtml(post.content);
    if (!articleText(post).trim()) throw new Error('Article content is required.');
  }
...
export function articleBody(post) {
  return `<main id="main" class="article"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a> / <a href="/blog">Blog</a> / <span>${escape(post.title)}</span></nav><span class="eyebrow">${escape(post.category || 'Celebration guides')}</span><h1>${escape(post.title)}</h1><p class="lead">${escape(post.excerpt)}</p><p class="muted">By ${escape(post.author)} · <time datetime="${post.publishedAt || post.updatedAt}">${new Date(post.publishedAt || post.updatedAt).toLocaleDateString('en-IN', { dateStyle: 'long', timeZone: 'Asia/Kolkata' })}</time> · ${Math.max(1, Math.ceil(articleText(post).split(/\s+/).length / 200))} min read</p>${post.cover ? `<img class="cover" src="${escape(post.cover)}" alt="${escape(post.coverAlt)}" width="1200" height="675" fetchpriority="high">` : ''}<article class="prose">${articleHtml(post)}</article><aside class="callout"><h2>Bring your celebration to life</h2><p>Discover venues, decorators, caterers and more with PartyClub.</p><a class="button" href="/#download">Explore the app</a></aside><a href="/blog">← All articles</a></main>`;
}
